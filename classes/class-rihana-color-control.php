...
class Rihana_Custom_Colors {
	...
	public function custom_color_variables() {
		if ( 'd1e4dd' !== strtolower( get_theme_mod( 'background_color', 'D1E4DD' ) ) ) {
			wp_add_inline_style( 'rihana-style', $this->generate_custom_color_variables() );
		}
	}

	public function editor_custom_color_variables() {
		wp_enqueue_style(
			'rihana-custom-color-overrides',
		...
		$background_color = get_theme_mod( 'background_color', 'D1E4DD' );
		if ( 'd1e4dd' !== strtolower( $background_color ) ) {
			wp_add_inline_style( 'rihana-custom-color-overrides', $this->generate_custom_color_variables( 'editor' ) );
		}
	}
